...
import { addBlogPost } from '../actions/post';
...
class BlogEditPage extends React.Component{
    ...
    postBlog(){
        ...
        if(this.state.input_title === '' || this.state.input_description === '' || 
        this.state.filter.length === 0){
            ...
            let blog_description = this.state.input_description;
            if(blog_description.length > 120){
                blog_description = this.state.input_description.substring(0, 115) + "...";
            }

            blogObj = {
                title: this.state.input_title,
                filters: this.state.filter,
                blog_pic: this.state.image,
                description: blog_description,
                blog_content: this.state.input_description,
            ...
            for(let key in blogObj){
                if(key === "filters"){
                    let filter_arr = blogObj[key];
                    for(let j = 0; j < filter_arr.length; j++){
                        form_data.append(key, filter_arr[j]);
                    }
                } else {
                    form_data.append(key, blogObj[key]);
                }
            }

            addBlogPost(form_data, this);           
        }
        ...
        return(
            <div className='whole_page'>
                ...
                <div className='blog_edit_header'>
                    <h1 className='header_text1'>Create your own blog</h1>
                    <span className='header_text2'>
                        Please fill all required (<span className='required'>*</span>) inforamtion below to create your own blog
                    </span>
                </div>
                <div className="blog_title">
                    <strong>Blog title (Max 30 characters) <span className='required'>*</span></strong>
...
　　　　             <input type="file" id="blog_pic_input" name="blog_pic" 
                    accept="image/*" onChange={this.uploadPic}/>
                    <label htmlFor="blog_pic_input">
                ...
                <div className='blog_text'>
                    <strong>Text content <span className='required'>*</span></strong>
                    <form className="blog_text_input" noValidate autoComplete="off">
                        <div>
                            <TextField
                                id="filled-textarea"
                                label="Insert your text content here"
                                placeholder=""
                                multiline
                                variant="outlined"
                                fullWidth
                ...
                <div className='blog_button'>
                    <div className='post_button'>
                        <Button variant="contained" color="primary" onClick={(e) => this.postBlog()}>
                            Post
                        </Button>
                        <Dialog
